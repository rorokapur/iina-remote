...
const COMMANDS = new Set<RemoteCommand>([
  "pause",
  "resume",
  "togglePause",
  "previousTrack",
...
function initGlobal() {
  ...
  function sanitizePlaylist(value: any) {
  ...
  function sanitizeIncomingState(raw: any) {
  ...
  function samePlaylist(a: any, b: any) {
  ...
  function diffState(next: any, prev: any) {
    const changed: any = {};
  ...
  function postToPlayers(command: RemoteCommand, value: any = null) {
  ...
  iina.ws.onMessage((conn, rawMessage) => {
    ...
    const command = parsed.command as RemoteCommand;
  ...
  iina.global.onMessage("remote-state", (rawState: any, playerID?: string) => {
    if (playerID) {
