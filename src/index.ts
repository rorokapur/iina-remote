...
function initPlayer() {
  ...
  let pendingState: Record<string, any> = {};
  let flushTimer: any = null;

  function reportError(scope: string, error: any) {
  ...
  function safeBoolean(name: string, fallback: boolean) {
    try {
      return Boolean(iina.mpv.getNative<boolean>(name));
    } catch {
      return fallback;
    }
  }

  function safeString(name: string, fallback: string) {
    try {
  ...
  function queueState(partial: Record<string, any>) {
  ...
  function displayNameFromPath(value: string) {
  ...
  function withGuard(scope: string, fn: () => void) {
    try {
      fn();
    } catch (e) {
      reportError(scope, e);
  ...
  function handleRemoteCommand(raw: any) {
    if (!raw || typeof raw !== "object") return;
    const command = raw.command as RemoteCommand;
  ...
  iina.global.onMessage("remote-command", (message: any) =>
